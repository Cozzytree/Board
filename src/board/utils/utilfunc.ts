...
import type { connection } from "../shapes/shape_types";
import type { BoxInterface, Point, resizeDirection } from "../types";
...
function routeOrthogonalRobust(
    ...
    candidates.push([A, Aout, { x: Aout.x, y: outerBottom }, { x: Bout.x, y: outerBottom }, Bout, B]);
...
function routeOrthogonalRobustDynamic(
    ...
    for (const sa of sides) {
        for (const sb of sides) {
            ...
            if (!cand) continue;
            if (!best || cand.bends < best.bends || (cand.bends === best.bends && cand.len < best.len)) {
                best = cand;
            }
        }
    }
...
function helperEvent(e: React.MouseEvent, attrName: string, fn: (v: any) => void) {
    const target = e.target as HTMLElement;
    const attr = target.getAttribute(attrName);
    if (attr) {
        fn(attr);
    }
}
