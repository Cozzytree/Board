 ...
 */
export async function saveLibraryItems(items: SavedLibraryItem[] | SavedLibraryItem): Promise<void> {
...
export async function getAllLibraryItems(): Promise<SavedLibraryItem[]> {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
            const result = (event.target as IDBRequest).result as SavedLibraryItem[];
            db.close();
            resolve(result || []);
        };
