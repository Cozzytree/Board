...
class PlainLine extends Line {
  ...
  draw({ ctx, resize = false }: DrawProps): void {
    ...
    // ---- GAP FOR ARROWS ----
    let startOffset = this.arrowS ? 10 * Math.cos(Math.PI / 6) + 2 : 0;
    let endOffset = this.arrowE ? 10 * Math.cos(Math.PI / 6) + 2 : 0;

    if (startOffset + endOffset > length) {
      const ratio = length / (startOffset + endOffset);
      startOffset *= ratio;
      endOffset *= ratio;
    }
    ...
    if (hasText) {
      ...
      // ---- DRAW LINE WITH GAP ----
      if (startOffset < gapStartDist) {
        context.beginPath();
        context.moveTo(p0.x + ux * startOffset, p0.y + uy * startOffset);
        context.lineTo(gapStart.x, gapStart.y);
        context.stroke();
      }

      if (length - endOffset > gapEndDist) {
        context.beginPath();
        context.moveTo(gapEnd.x, gapEnd.y);
        context.lineTo(p1.x - ux * endOffset, p1.y - uy * endOffset);
        context.stroke();
      }
    ...
    if (this.arrowS) {
      this.renderArrow({ arrowLength: 10, ctx: context, endPoint: p0, startPoint: p1, color: this.fill });
    }
    if (this.arrowE) {
      this.renderArrow({ arrowLength: 10, ctx: context, endPoint: p1, startPoint: p0, color: this.fill });
    }
