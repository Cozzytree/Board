...
globalThis.canvasInteractor = globalThis.canvasInteractor || {
  ...
  tick(time) {
    ...
    for (const instance of canvasInteractor.instances) {
      instance.notify("tick", { t: time, dt: (time - instance.t) / 1e3, dirty: instance.dirty });
      instance.t = time;
      instance.dirty = false;
    }
    canvasInteractor.rafid = requestAnimationFrame(canvasInteractor.tick);
  },
  add(instance) {
    canvasInteractor.instances.push(instance);
    if (canvasInteractor.instances.length === 1)
  ...
  remove(instance) {
    ...
    if (canvasInteractor.instances.length === 0) cancelAnimationFrame(canvasInteractor.rafid);
  },
  fpsCount(time) {
    if (time - canvasInteractor.fpsOrigin > 1e3) {
      const fps = ~~((canvasInteractor.frames * 1e3) / (time - canvasInteractor.fpsOrigin) + 0.5);
  ...
  prototype: {
    constructor(ctx, { x = 0, y = 0, scl = 1, cartesian = false }) {
      this.ctx = ctx;
      this.view = { x: x, y: y, scl: scl, cartesian: cartesian };
      this.evt = {
        type: false,
        x: -2,
        y: -2,
    ...
    handleEvent(e) {
      if (e.type in this && (e.isPrimary || e.type === "wheel")) {
        const bbox = (e.target.getBoundingClientRect && e.target.getBoundingClientRect()) || {
            left: 0,
            top: 0,
          },
        ...
        if (this.isDefaultPreventer(e.type)) e.preventDefault();
        this[e.type]();
    ...
    pointermove() {
      ...
      if (this.evt.btn === 1) {
        this.evt.dxusr = this.evt.dx / this.view.scl;
        this.evt.dyusr = this.evt.dy / this.view.scl;
        if (!this.evt.hit) {
          this.evt.type = "pan";
        } else this.evt.type = "drag";
      }
      this.dirty = true;
    },
    pointerdown() {
    ...
    pointerup() {
      this.evt.type =
        this.evt.x === this.evt.xbtn && this.evt.y === this.evt.ybtn ? "click" : "pointerup";
      this.evt.xbtn = this.evt.x;
      this.evt.ybtn = this.evt.y;
      this.evt.hit = false;
    },
    ...
    endTimer() {
      this.notify("timerend", this.t / 1e3);
      canvasInteractor.remove(this);
      return this;
    },
    notify(key, val) {
      if (this.signals[key]) for (let hdl of this.signals[key]) hdl(val);
      return this;
    },
    on(key, handler) {
      if (Array.isArray(key)) for (let k of key) this.on(k, handler);
      else (this.signals[key] || (this.signals[key] = [])).push(handler);
      return this;
    },
    remove(key, handler) {
